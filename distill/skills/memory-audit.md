...
- Before writing the final `.agent/handoff_letter.md`.
...
2. The script does the following:
   - Read `.agent/session_state/<session_id>/candidate_memory.jsonl`.
   ...
   - If all three exist, merge into `.agent/knowledge_distill.md` in the format:
     ```
   ...
   - If `.agent/knowledge_distill.md` exceeds 8KB, run a distillation pass:
     - merge duplicates
     - abstract concrete paths into patterns
     - archive evicted entries to `.agent/loop_state_archive.md`
   - Clear the processed candidates from `.agent/session_state/<session_id>/candidate_memory.jsonl`.
3. You may also read the audit output from the script and append any high-level judgment to `.agent/handoff_letter.md`.
